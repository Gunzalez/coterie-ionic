...
export class ParticipantsPage {
    ...
    onAddParticipant(contact){
        this.contactList.map((contactInList, index) => {
            if(isEquivalent(contactInList, contact)){
                ...
                this.removeFromCopy(participant);
                this.groupContacts();
            }
        });
    }

    removeFromCopy(participant){
        this.contactListCopy.map((contact, index) => {
            if(isEquivalent(contact, participant)){
                this.contactListCopy.splice(index, 1).pop();
            }
        })
    }
    ...
    onRemoveParticipant(index){
        ...
        this.contactListCopy.push(Object.assign({}, participant));
        this.groupContacts();
    }
